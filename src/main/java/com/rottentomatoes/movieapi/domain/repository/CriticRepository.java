    ...
    public Iterable<Critic> findAll(RequestParams requestParams) {
        ...
        if(requestParams.getFilters() != null) {
            if (requestParams.getFilters().containsKey("index-search")) {
                ...
                if(criticIds.size() > 0){
                    critics = new MetaDataEnabledList<>((List<Critic>) preEmsClient.callPreEmsList(selectParams, "critic", null, TypeFactory.defaultInstance().constructCollectionType(List.class,  Critic.class)));
                ...
                critics = new MetaDataEnabledList<>((List<Critic>) preEmsClient.callPreEmsList(selectParams, "critic", null, TypeFactory.defaultInstance().constructCollectionType(List.class,  Critic.class)));
                critics.setMetaInformation((RelatedMetaDataInformation) preEmsClient.callPreEmsEntity(selectParams, "critic", "/meta", RelatedMetaDataInformation.class));
            }
        }
